...
export function StatsWidget13({ className, symbolShape, baseColor, id, totalSuppliers }) {
  ...
  useEffect(() => {
    ...
    const options = getChartOption(layoutProps, height, totalSuppliers);
    const chart = new ApexCharts(element, options);
    chart.render();
    return function cleanUp() {
      chart.destroy();
    };
  }, [layoutProps, totalSuppliers]);
        ...
        <div className="card-body p-0">
          <div className="d-flex align-items-center justify-content-between card-spacer flex-grow-1">
            ...
            <div className="d-flex flex-column text-right">
              ...
              <span className="text-muted font-weight-bold mt-2">Suppliers</span>
            </div>
          </div>
          <div
            id={id}
...
function getChartOption(layoutProps, height, totalSuppliers) {
  const options = {
    series: [
      {
        name: "",
        data: [0, totalSuppliers],
      },
    ],
    chart: {
      type: "area",
